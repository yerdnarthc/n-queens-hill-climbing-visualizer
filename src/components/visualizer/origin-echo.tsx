 ...
 *  2. Ghost — a literal `Crown` silhouette at partial opacity that shrinks
 *     and dissolves. Shape (not just color) carries the meaning, so it
 *     stays legible for colorblind users and at a glance during fast play.
  ...
  if (reducedMotion) {
    ...
    return (
      ...
      >
        ...
        />
        <Crown
          className="relative h-1/2 w-1/2 fill-current text-improving-deep opacity-40"
          aria-hidden="true"
        />
  ...
  return (
    ...
    >
      ...
      <motion.div
        ...
        transition={{ duration: seconds, ease: 'easeOut' }}
      >
        <Crown className="h-1/2 w-1/2 fill-current text-improving-deep" />
      </motion.div>
      {/* Origin-row label */}
      <span className="absolute right-0.5 bottom-0.5 rounded-sm bg-card px-1 font-mono text-[8px] leading-4 font-bold text-improving-deep ring-1 ring-improving-deep/60">
        R{move.fromRow + 1}
